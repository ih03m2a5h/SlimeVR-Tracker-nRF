...
static const float accel_sensitivity_32 =
    32.0f / ((uint32_t)2 << 30); // 32G forced
static const float gyro_sensitivity_32 =
    4000.0f / ((uint32_t)2 << 30); // 4000dps forced
...
int icm45_init(float clock_rate, float accel_time, float gyro_time,
  ...
  if (clock_rate > 0) {
    ...
    err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
                              ...
                              0x06); // override pin 9 to CLKIN
    err |= ssi_reg_update_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_RTC_CONFIG,
                               0x20, 0x20); // enable external CLKIN
    //		err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
    //ICM45686_RTC_CONFIG, 0x23); // enable external CLKIN (0x20, default
    //register value is 0x03)
  }
  uint8_t ireg_buf[3];
  ireg_buf[0] = ICM45686_IPREG_BAR; // address is a word, icm is big endian
  ireg_buf[1] = ICM45686_IPREG_BAR_REG_58;
  ireg_buf[2] =
      0xD9 & ~0x48; // disable internal pull resistors for AP pins (pin 13, 12)
  ...
  ireg_buf[2] =
      0xB6 &
      ~0x92; // disable internal pull resistors for AP pins (pin 7, 1, 14)
  err |= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, ICM45686_IREG_ADDR_15_8,
                         ireg_buf, 3); // write buffer
  ...
  err |= icm45_update_odr(accel_time, gyro_time, accel_actual_time,
                          gyro_actual_time);
  //	k_msleep(50); // 10ms Accel, 30ms Gyro startup
  k_msleep(1); // fuck i dont wanna wait that long
  err |= ssi_reg_write_byte(
      SENSOR_INTERFACE_DEV_IMU, ICM45686_FIFO_CONFIG0,
      0x80 | 0b000111); // set FIFO stop-on-full mode, set FIFO depth to 2K
                        // bytes (see AN-000364)
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_FIFO_CONFIG3,
                            0x0F); // begin FIFO stream, hires, a+g
  if (err)
...
void icm45_shutdown(void) {
  ...
  last_gyro_odr = 0xff;  // reset last odr
  int err =
      ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_REG_MISC2,
                         0x02); // Don't need to wait for ICM to finish reset
                                //	k_msleep(1);
  // TODO: not working
  //	uint8_t ireg_buf[3];
  //	ireg_buf[1] = ICM45686_IPREG_BAR_REG_60;
  //	ireg_buf[2] = 0x6D & ~0x05; // set internal pull down resistors for AP
  //pins (pin 10, 7) 	err |= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU,
  ...
  //ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, ICM45686_IREG_ADDR_15_8, ireg_buf,
  //3); // write buffer
  if (err)
    LOG_ERR("Communication error");
}

void icm45_update_fs(float accel_range, float gyro_range,
...
int icm45_update_odr(float accel_time, float gyro_time,
                     float *accel_actual_time, float *gyro_actual_time) {
  int ODR;
  ...
  if (last_accel_odr == ACCEL_ODR &&
      last_gyro_odr == GYRO_ODR) // if both were already configured
    return 1;

  int err = 0;
  // only if the power mode has changed
  if (last_accel_odr == 0xff || last_gyro_odr == 0xff ||
      (last_accel_odr == 0 ? 0 : 1) != (ACCEL_ODR == 0 ? 0 : 1) ||
      (last_gyro_odr == 0 ? 0 : 1) !=
          (GYRO_ODR == 0 ? 0 : 1)) { // TODO: can't tell difference between gyro
                                     // off and gyro standby
    err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_PWR_MGMT0,
                              GYRO_MODE << 2 |
                                  ACCEL_MODE); // set accel and gyro modes
    k_busy_wait(250); // wait >200us // TODO: is this needed?
  }
  last_accel_odr = ACCEL_ODR;
  last_gyro_odr = GYRO_ODR;

  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_ACCEL_CONFIG0,
                            ACCEL_UI_FS_SEL << 4 |
                                ACCEL_ODR); // set accel ODR and FS
  err |=
      ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_GYRO_CONFIG0,
                         GYRO_UI_FS_SEL << 4 | GYRO_ODR); // set gyro ODR and FS
  if (err)
    LOG_ERR("Communication error");
...
uint16_t icm45_fifo_read(uint8_t *data, uint16_t len) // TODO: check if working
{
  ...
  while (packets > 0 && len >= PACKET_SIZE) {
    uint8_t rawCount[2];
    err |= ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, ICM45686_FIFO_COUNT_0,
                          &rawCount[0], 2);
    packets =
    ...
    err |= ssi_burst_read_interval(SENSOR_INTERFACE_DEV_IMU, ICM45686_FIFO_DATA,
...
void icm45_accel_read(float a[3]) {
  uint8_t rawAccel[6];
  int err = ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, ICM45686_ACCEL_DATA_X1_UI,
                           &rawAccel[0], 6);
  if (err)
    LOG_ERR("Communication error");
  for (int i = 0; i < 3; i++) // x, y, z
...
void icm45_gyro_read(float g[3]) {
  uint8_t rawGyro[6];
  int err = ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, ICM45686_GYRO_DATA_X1_UI,
                           &rawGyro[0], 6);
  if (err)
    LOG_ERR("Communication error");
  for (int i = 0; i < 3; i++) // x, y, z
...
int icm45_temp_read(float *data) {
  uint8_t rawTemp[2];
  int err = ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, ICM45686_TEMP_DATA1_UI,
                           &rawTemp[0], 2);
  if (err) {
    LOG_ERR("Communication error");
    return -1;
...
uint8_t icm45_setup_DRDY(uint16_t threshold) {
  ...
  buf[1] = threshold >> 8;
  int err = ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, ICM45686_FIFO_CONFIG1_0,
                            buf, 2);
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_INT1_CONFIG0,
...
uint8_t icm45_setup_WOM(void) // TODO: check if working
{
  ...
  int err = ssi_reg_read_byte(
      SENSOR_INTERFACE_DEV_IMU, ICM45686_INT1_STATUS0,
      &interrupts); // clear reset done int flag // TODO: is this needed
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_INT1_CONFIG0,
                            0x00); // disable default interrupt (RESET_DONE)
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_ACCEL_CONFIG0,
                            ACCEL_UI_FS_SEL_8G << 4 |
                                ACCEL_ODR_200Hz); // set accel ODR and FS
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_PWR_MGMT0,
                            ACCEL_MODE_LP); // set accel and gyro modes
  ...
  //	err |= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU,
  //ICM45686_IREG_ADDR_15_8, ireg_buf, 3); // write buffer
  ireg_buf[0] = ICM45686_IPREG_TOP1;
  ...
  err |= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, ICM45686_IREG_ADDR_15_8,
                         ireg_buf, 5); // write buffer
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_TMST_WOM_CONFIG,
                            0x14); // enable WOM, enable WOM interrupt
  err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM45686_INT1_CONFIG1,
                            0x0E); // route WOM interrupt
  if (err)
    LOG_ERR("Communication error");
  return NRF_GPIO_PIN_PULLUP << 4 | NRF_GPIO_PIN_SENSE_LOW; // active low
}
...
int icm45_ext_passthrough(bool passthrough) {
  ...
  if (passthrough)
    err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
                              ...
                              0x18); // AUX1_MODE_OVRD, AUX1 in I2CM Bypass
  else
    err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
                              ICM45686_IOC_PAD_SCENARIO_AUX_OVRD,
                              0x00); // disable overrides
  if (err)
    LOG_ERR("Communication error");
  return 0;
}
...
const sensor_imu_t sensor_imu_icm45686 = {
    *icm45_init,         *icm45_shutdown,

    *icm45_update_fs,    *icm45_update_odr,

    *icm45_fifo_read,    *icm45_fifo_process,   *icm45_accel_read,
    *icm45_gyro_read,    *icm45_temp_read,

    *icm45_setup_DRDY,   *icm45_setup_WOM,

    *imu_none_ext_setup, *icm45_ext_passthrough};
