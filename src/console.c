...
#else
K_THREAD_DEFINE(console_thread_id, 4096, console_thread, NULL, NULL, NULL,
...
static const char *meow_suffixes[] = {" :3",  " :3c",  " ;3",  " ;3c", " x3",
                                      " x3c", " X3",   " X3c", " >:3", " >:3c",
                                      " >;3", " >;3c", ""};

static uint8_t meow_colors[] = {212, 176, 177};

void console_thread_create(void) {
#if USB_EXISTS
  k_thread_create(&console_thread_id, console_thread_id_stack,
...
static void print_sensor(void) {
  ...
  if (CONFIG_1_SETTINGS_READ(CONFIG_1_SENSOR_USE_6_SIDE_CALIBRATION)) {
    ...
    for (int i = 0; i < 3; i++)
      printk("%.5f %.5f %.5f %.5f\n", (double)retained->accBAinv[0][i],
             (double)retained->accBAinv[1][i], (double)retained->accBAinv[2][i],
  ...
  for (int i = 0; i < 3; i++)
    printk("%.5f %.5f %.5f %.5f\n", (double)retained->magBAinv[0][i],
           (double)retained->magBAinv[1][i], (double)retained->magBAinv[2][i],
...
static void print_battery_tracker(void) {
  ...
  if (last_min >= 0 && last_max >= 0 && last_runtime > 0) {
    printk("\nLast discharge cycle: %.2f%% -> %.2f%% (Raw %.2f%% -> %.2f%%)\n",
           ...
           (double)last_calibrated_min / 100.0, (double)last_max / 100.0,
...
static void print_meow(void) {
  ...
  ticks %= ARRAY_SIZE(meows) * ARRAY_SIZE(meow_punctuations) *
           ARRAY_SIZE(meow_suffixes) *
           ARRAY_SIZE(meow_colors); // silly number generator
  uint8_t meow = ticks / (ARRAY_SIZE(meow_punctuations) *
                          ARRAY_SIZE(meow_suffixes) * ARRAY_SIZE(meow_colors));
  ticks %= ARRAY_SIZE(meow_punctuations) * ARRAY_SIZE(meow_suffixes) *
           ARRAY_SIZE(meow_colors);
  uint8_t punctuation =
  ...
  printk("\033[38;5;%d;1m%s%s\033[0;38;2;255;255;255m%s\033[0m\n",
         meow_colors[color], meows[meow], meow_punctuations[punctuation],
...
static void console_thread(void) {
  ...
  while (1) {
    printk("console_thread: waiting for input...\n");
#if CONFIG_CONSOLE_GETLINE
    ...
    if (line == NULL) {
      printk("console_thread: line is NULL\n");
      k_msleep(10);
      continue;
    }

    printk("console_thread: got line: %p\n", line);
    char *argv[5] = {NULL}; // command and 4 args
    size_t argc = parse_args(line, argv, ARRAY_SIZE(argv));
    printk("console_thread: parsed %d args\n", argc);
    if (argc == 0)
      continue;
    if (argc > 0)
    ...
    if (strcmp(argv[0], command_help) == 0) {
      ...
      print_uptime(uptime - retained->uptime_latest + retained->uptime_sum,
                   "Accumulated");
      ...
      uint8_t buf[17];
      snprintk(buf, 17, "%0*llx", (int)strlen(argv[1]), addr);
      ...
      if (argc != 3) {
        ...
        sys_write(SETTINGS_ID, NULL, retained->settings,
                  sizeof(retained->settings));
        k_free(tmp);
